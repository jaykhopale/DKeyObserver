...
public class DkeyNotes extends BaseActivity implements IGetNotesValuesListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		super.addtitleBar(layout, R.layout.dkeyobservetitlebar, "Notes", true,
				R.id.informationID, R.id.doneButtonID);
...

	{
...

		{
			...
			latValue.setText(Double.valueOf(twoDForm.format(loc.getLatitude()))
					.toString());

			longiValue.setText(Double.valueOf(
					twoDForm.format(loc.getLongitude())).toString());
	...
	public ArrayList<String> moreFieldValues() {
		...
		moreFields.add(key1.getText().toString() + " : "
				+ value1.getText().toString());
		moreFields.add(key2.getText().toString() + " : "
				+ value2.getText().toString());
		moreFields.add(key3.getText().toString() + " : "
				+ value3.getText().toString());
		return moreFields;
	}
}
